...
const DiaryTextArea = () => {
  ...
  return (
    <div className=" flex flex-col gap-8px-col-m md:w-552px-row md:gap-8px-col">
      ...
      <div className="flex flex-col px-8px-col-m py-16px-row-m gap-8px-col-m md:gap-8px-col border-[#A1A1A1] border rounded-[8px] w-335px-row-m h-135px-col-m bg-white md:w-552px-row md:h-160px-col md:py-8px-col  md:px-16px-row focus-within:border-gray-900">
        <Controller
          name="diaryContent"
          control={control}
          render={({ field }) => (
            <textarea
              className="text-font-color w-full h-102px-col-m md:h-148px-col rounded-[8px] resize-none custom-scrollbar text-14px-m md:text-16px outline-none"
        ...
        <div className="relative h-12px-col-m md:h-12px-col rounded-[8px]">
          <div className="rounded-[8px] absolute bottom--1 md:bottom-0 right-1 text-gray-400 text-12px-m md:text-16px">
            {charCount}/500
          </div>
        </div>
      </div>
