      ...
      <div className="flex flex-col gap-32px-col px-4 pt-4 pb-6 md:px-96px-row md:pt-36px-col md:pb-56px-col rounded-5xl bg-sign-up border-4 border-border-color">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex flex-col gap-2">
            <label
              htmlFor="textarea"
              className={`text-16px-m md:text-18px font-medium ${
                isSubmitted
            ...
            <div className="flex flex-col mb-[42px]">
              <div
                className={`flex flex-col border-[1px] rounded-lg px-16px-row py-8px-col bg-white  ${
                  isSubmitted
                    ? errors.textarea || textareaValue.length > maxLength
                      ? 'border-[#F02222]'
                      : 'border-[#25B18C]'
...
ex)일기의 날짜가 입력이 안돼요.`}
              ...
              {errors.textarea && (
                <span className="text-error-color md:text-18px text-14px-m mt-[6px]">{errors.textarea.message}</span>
              )}
            </div>
          </div>
